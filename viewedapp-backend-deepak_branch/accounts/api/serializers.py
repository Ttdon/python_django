...
class UserDetailsForSuggestions(ModelSerializer):
	...
	def get_id(self, instance):
		return instance.viewed_to.id
	def get_first_name(self , instance):
		return instance.viewed_to.first_name
	def get_last_name(self, instance):
		return instance.viewed_to.last_name
	def get_profile_image(self, instance):
		if instance.viewed_to.profile_image:
			return  instance.viewed_to.profile_image.url
		return None
	class Meta:
...
class ViewingAndViewersSerializer(ModelSerializer):
	...
	def get_user_detail(self, instance):
		data = IncomingRequestsUsersSerializer(instance.viewed_by, context={'request':self.context.get('request')}).data
		return data

	def get_viewed_by(self, instance):
		viewing = self.context['request'].user.viewing.all().values_list('viewed_to',flat=True)
		qs = ViewingAndViewers.objects.filter(viewed_to = instance.viewed_by, viewed_by__in = viewing).values(name=F('viewed_by__first_name'))
		if not qs.exists():
			return []
		else:
			return qs
...
class SuggestionSerializer(ModelSerializer):
	...
	def get_viewed_by(self, instance):
		user_viewings = self.context['request'].user.viewing.all().values_list('viewed_to',flat=True)
		viewed_by_list = ViewingAndViewers.objects.filter(status='2', viewed_by__in = user_viewings, viewed_to = instance).values(name=F('viewed_by__first_name'))

		if not viewed_by_list.exists():
			return []
		else:
			return viewed_by_list

	def get_status(self, instance):
		qs = ViewingAndViewers.objects.filter(viewed_to=instance, viewed_by=self.context['request'].user)
		# 1 - requested
		# 2 - no relation
	...
	class Meta:
		model = User
		fields = ['id', 'first_name', 'last_name', 'profile_image','profile_views_count', 'status', 'total_viewers', 'viewed_by']
...
class UserProfileViewByUsersSerializer(ModelSerializer):
	...
	def get_viewed_by(self, instance):
		user_viewings = self.context['request'].user.viewing.all().values_list('viewed_to',flat=True)
		viewed_by_list = ViewingAndViewers.objects.filter(status='2', viewed_by__in = user_viewings, viewed_to = instance).values(name=F('viewed_by__first_name'))
		# viewed_by_list = ViewingAndViewers.objects.filter(status='2', viewed_by__in = user_viewings, viewed_to = instance).values(first_name=F('viewed_by__first_name'), last_name=F('viewed_by__last_name'))
	...
	# 	sugg_qs = instance.viewing.filter(~Q(viewed_to=self.context.get('request').user) & ~Q(viewed_to__in = self.context['request'].user.viewing.all().values_list('viewed_to')) ).select_related('viewed_to')
	# 	# remove self and mutual viwing
	# 	data = UserDetailsForSuggestions(sugg_qs, many=True).data
	# 	return data

	def get_suggestions(self,instance):
		all_viewing = self.context.get('request').user.viewing.filter(status='2').values_list('viewed_to', flat=True)  # viewing of user
...
class SearchResultSerializer(ModelSerializer):
	...
	class Meta:
		model = User
		fields = ['id', 'first_name', 'last_name', 'profile_image','viewed_by','total_viewers','profile_views_count', 'status']
