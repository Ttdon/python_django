...
from .task import send_email_verify_mail, create_user_node, send_phone_verify_otp ,delete_user_node, update_user_node
...
class GetViewingListApiView(APIView):
	...
	def get(self, request,*args, **kwargs):
		...
		qs = ViewingAndViewers.objects.filter(viewed_by=request.user, status="2")
		viewing_list = qs.values(first_name=F('viewed_to__first_name'),
				last_name=F('viewed_to__last_name'), profile_image=F('viewed_to__profile_image'),
				user_id=F('viewed_to__id'))
		all_viewing = qs.values_list('viewed_to__id', flat=True)

		if frnd_type == '1':
			for obj in viewing_list:
				obj['profile_image'] = '/media/'+obj['profile_image']
				obj['viewed_by'] = ViewingAndViewers.objects.filter(status='2', viewed_to=obj['user_id'] , viewed_by__in =all_viewing).values(first_name=F('viewed_by__first_name'),
					last_name=F('viewed_by__last_name'))
		...
		if frnd_type=='2':
			...
			# get viewers list
			qs = ViewingAndViewers.objects.filter(viewed_to = request.user, status="2")
			viewers_list = qs.values(first_name=F('viewed_by__first_name'),
					last_name=F('viewed_by__last_name'), profile_image=F('viewed_by__profile_image'),
					user_id=F('viewed_by__id'))

			all_viewers = qs.values_list('viewed_by__id', flat=True)

			for obj in viewers_list:
				obj['profile_image'] = '/media/' + obj['profile_image']
				# viewed by
				obj['viewed_by'] = ViewingAndViewers.objects.filter(status='2', viewed_to=obj['user_id'],
																	viewed_by__in=all_viewing).values(
					first_name=F('viewed_by__first_name'),
					last_name=F('viewed_by__last_name'))

				# status to user
				viewing_status_qs = ViewingAndViewers.objects.filter(viewed_to__id=obj['user_id'], viewed_by=user,
																	 status__in=['1', '2'])  # following
				viewers_status_qs = ViewingAndViewers.objects.filter(viewed_by__id=obj['user_id'], viewed_to=user,
...
class ViewOtherUserFriendsAPIView(APIView):
	...
	def get(self, request ,*args , **kwargs):
		...
		user = request.user
		user_viewing_list = ViewingAndViewers.objects.filter(viewed_by=request.user, status="2").values_list('viewed_to__id', flat=True)

		# get viewers list
		qs = ViewingAndViewers.objects.filter(viewed_to=other_user_obj, status="2").exclude(viewed_by=user)
		viewers_list = qs.values(first_name=F('viewed_by__first_name'),
								 last_name=F('viewed_by__last_name'), profile_image=F('viewed_by__profile_image'),
								 user_id=F('viewed_by__id'))
		...
		for obj in viewers_list:
			# viewed by
			obj['viewed_by'] = ViewingAndViewers.objects.filter(status='2', viewed_to=obj['user_id'],
																viewed_by__in=user_viewing_list).values(
				first_name=F('viewed_by__first_name'),
				last_name=F('viewed_by__last_name'))
		...
		# get viewing list
		qs = ViewingAndViewers.objects.filter(viewed_by=other_user_obj, status="2").exclude(viewed_to=user)
		viewing_list = qs.values(first_name=F('viewed_to__first_name'),
								 last_name=F('viewed_to__last_name'), profile_image=F('viewed_to__profile_image'),
								 user_id=F('viewed_to__id'))
...
class SendFollowRequestToWiewersAPIView(APIView):
	...
	def post(self, request):
		...
		try:
			obj, created = ViewingAndViewers.objects.get_or_create(viewed_by=request.user,
																   viewed_to_id=user_id)  # status False
			if not created:
				obj.delete()
		except:
	...
	def post(self, request):
		...
		if serializer.is_valid():
			...
			if request.user.id==int(user_id):
				return Response({'message': 'You can not follow self'}, 400)
			try:
				obj, created = ViewingAndViewers.objects.get_or_create(viewed_by=request.user, viewed_to_id=user_id)  #status False
				if not created:
					obj.delete()
			...
			if (name_key == '' or name_key == None) and (is_profile== '0' or is_profile == None):
				viewing_viewers_qs = ViewingAndViewers.objects.filter(viewed_to=user, status='1')
				viewing_viewers_data = ViewingAndViewersSerializer(viewing_viewers_qs, many=True,
																   context={'request': request}).data

				all_viewing = user.viewing.filter(status='2').values_list('viewed_to', flat=True)  # viewing of user
			...
			if (name_key == '' or name_key == None) and (is_profile== '1'):
				...
				viewing_status_qs = ViewingAndViewers.objects.filter(viewed_to=user_obj, viewed_by=user,
																	 status__in=['1', '2'])  # following
				viewers_status_qs = ViewingAndViewers.objects.filter(viewed_by=user_obj, viewed_to=user,
																	 status__in=['1', '2'])  # followers
				request_id = None
				if viewing_status_qs.exists():
					viewing_status = viewing_status_qs.first().status
				...
				# profile views count
				obj, created = ProfileViewsCountRecord.objects.get_or_create(profile_viewed_by=user,
																			 profile_viewed_to=user_obj)
				if created:
...
class AcceptOrCancelFollowRequestAPIView(APIView):
	...
	def post(self, request):
		...
		if serializer.is_valid():
			try:
				obj = ViewingAndViewers.objects.get(id = data['request_id'], status='1', viewed_to = request.user)
			except:
...
class ShowFriendsRequestAndSuggestions(APIView):
	...
	def get(self, request):
		...
		# viewing_frnd = ViewingAndViewers.objects.filter(viewed_by__in=all_viewing, status='2').select_related('viewed_to').values_list( 'viewed_to', flat=True)  # all viewing users of vieweing user
...
class ViewUserProfileAPIView(APIView):
	...
	def get(self, request, *args , **kwargs):
		...
		viewing_status_qs = ViewingAndViewers.objects.filter(viewed_to=user_obj, viewed_by=user, status__in=['1', '2'])  #following
		viewers_status_qs = ViewingAndViewers.objects.filter(viewed_by=user_obj, viewed_to=user, status__in=['1', '2'])  #followers
		request_id =None
		if viewing_status_qs.exists():
			viewing_status = viewing_status_qs.first().status
		else:
		...
		data['request_id'] = request_id
		# profile views count
		obj, created = ProfileViewsCountRecord.objects.get_or_create(profile_viewed_by= user, profile_viewed_to=user_obj)
		if created:
			user_obj.profile_views_count = user_obj.profile_views_count +1
			user_obj.save()
...
class StopViewingAFriend(APIView):
	...
	def post(self,request):
		...
		if serializer.is_valid():
			...
			obj ,created = ViewingAndViewers.objects.get_or_create(viewed_to=user_obj, viewed_by=curr_user)
			if not created:
				status = obj.status
				if status == '1':
			...
			viewing_status_qs = ViewingAndViewers.objects.filter(viewed_to=user_obj, viewed_by=curr_user,
																 status__in=['1', '2'])  # following
			viewers_status_qs = ViewingAndViewers.objects.filter(viewed_by=user_obj, viewed_to=curr_user,
																 status__in=['1', '2'])  # followers
			request_id = None
			if viewing_status_qs.exists():
				viewing_status = viewing_status_qs.first().status
			...
			data['request_id'] = request_id
			# profile views count
			obj, created = ProfileViewsCountRecord.objects.get_or_create(profile_viewed_by=curr_user,
																		 profile_viewed_to=user_obj)
			if created:
				user_obj.profile_views_count = user_obj.profile_views_count + 1
				user_obj.save()
