...
class CompetetionHomePageSerializer(ModelSerializer):
    ...
    def get_is_viewed(self, instance):
        ...
        if user.is_authenticated:
            if instance.post.is_shared_post:
                viewed_qs = CompetitionViewedByUsers.objects.filter(post=instance.post.parent_main_post_id, viewed_by=user)
            else:
                viewed_qs = CompetitionViewedByUsers.objects.filter(post=instance.post, viewed_by=user)
            if viewed_qs.exists():
                return True
        return False
