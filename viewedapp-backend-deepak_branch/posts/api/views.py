...
class HomePageAPIView(ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        viewing = list(ViewingAndViewers.objects.filter(viewed_by = request.user, status='2').values_list('viewed_to', flat=True))
        #viewers = list(ViewingAndViewers.objects.filter(viewed_to = request.user, status='2').values_list('viewed_by', flat=True))
...
class CompetitionViewedByPostAPIView(APIView):
    permission_classes = (IsAuthenticated,)
    authentication_classes = [JSONWebTokenAuthentication]
    ...
    def post(self,request):
        ...
        try:
            ...
            if post.is_shared_post:
                ...
                main_post = post
            obj,created = CompetitionViewedByUsers.objects.get_or_create(post=main_post, viewed_by=request.user)
            if created:
                comp_obj = CompetitionPost.objects.get(post=main_post)
                comp_obj.viewed_by = F('viewed_by')+1
                comp_obj.save()
                CompetitionPost.objects.filter(post__parent_main_post_id=main_post,post__is_shared_post=True).update(viewed_by=comp_obj.viewed_by)
        ...
        try:
            ...
            if created:
                ...
                CompetitionPost.objects.create(post=share_post, entry_requirement = comp_obj.entry_requirement,no_of_winners=comp_obj.no_of_winners,
                                               ...
                                               video_thumbnail=comp_obj.video_thumbnail, viewed_by=comp_obj.viewed_by,entered_by=comp_obj.entered_by)
